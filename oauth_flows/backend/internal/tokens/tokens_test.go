...
func (suite *TokensTestSuit) TestProtectsFromTampering() {
	...
	correctToken := Must(GenerateTokenFor(purpose, originalPayload))
	correctTokenSegments := Must(GetTokenSegments(correctToken))
	originalDigest := correctTokenSegments.Digest

	manipulatedPayload := map[string]string{"userId": "456"}
	manipulatedToken := Must(GenerateTokenFor(purpose, manipulatedPayload))
	manipulatedTokenSegments := Must(GetTokenSegments(manipulatedToken))
	...
	isValid := Must(VerifyToken(purpose, token))
...
func (suite *TokensTestSuit) TestGetPayload() {
	...
	token := Must(GenerateTokenFor(purpose, payload))
